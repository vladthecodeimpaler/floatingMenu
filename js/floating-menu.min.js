...
FloatingMenu.prototype={init:function(options){this.selector=options.selector;this.menuReference=Math.random();this.$this=$(this).data('raw',this);this.$menuTemplate=$(this.markup).data('menuReference',this.menuReference);var $li=null;var $a=null;options.items.forEach(function(item){$li=$("<li></li>");if(item.action instanceof Function){$a=$("<a href='javascript:void(null);'>"+item.title+"</a>").click({plugin:this,callback:item.action,closeMenu:!0},this.actionHandler)}else{$a=$("<a href='"+item.action+"' target='_blank'>"+item.title+"</a>")}
if(item.icon!=undefined){var $span=$('<span></span>').addClass('fm-icon '+item.icon);$a.prepend($span)}
$a.data('menuReference',this.menuReference);$li.append($a);this.$menuTemplate.append($li)},this);$(document).on('click.show.'+this.menuReference,this.selector,$.proxy(this.calloutHandler,this))},actionHandler:function(event){event.data.callback(event);if(event.data.closeMenu){event.data.plugin.hide()}},calloutHandler:function(event){var $callout=$(event.target);if($callout.attr('data-fm-callout')=='true'){this.hide();return}
...
this.detachShowEvents();this.$this.trigger("beforeHide");(function(_self){_self.$menuTemplate=_self.$menuTemplate.detach();_self.$menuTemplate.attr('class',_self.mainClass);_self.isVisible=!1;_self.calloutRemoveIsCallout()}(this));this.$this.trigger("afterHide")},calloutRemoveIsCallout:function(attr){if(this.$callout!=null){this.$callout.removeAttr('data-fm-callout');this.$callout=null}},attachShowEvents:function(){var _self=this;$(document).on('click.hide.'+this.menuReference,function(event){if($(event.target).data!=_self.menuReference&&!$(event.target).is(_self.selector)){_self.hide()}})},detachShowEvents:function(){$(document).off('click.hide.'+this.menuReference)},destroy:function(){this.hide();$(document).off('click.show.'+this.menuReference)}}
