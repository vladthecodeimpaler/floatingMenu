...
this.detachShowEvents();this.$this.trigger("beforeHide");(function(_self){_self.$menuTemplate=_self.$menuTemplate.detach();_self.$menuTemplate.attr('class',_self.mainClass);_self.isVisible=!1;_self.calloutRemoveIsCallout()}(this));this.$this.trigger("afterHide")},calloutRemoveIsCallout:function(attr){if(this.$callout!=null){this.$callout.removeAttr('data-fm-callout');this.$callout=null}},attachShowEvents:function(){var _self=this;$(document).on('click.hide.'+this.menuReference,function(event){if($(event.target).data('menuReference')!=_self.menuReference){_self.hide()}})},detachShowEvents:function(){$(document).off('click.hide.'+this.menuReference)},destroy:function(){this.hide();$(document).off('click.show.'+this.menuReference)}}
